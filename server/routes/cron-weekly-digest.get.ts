...
export default defineEventHandler(async (event) => {
  ...
  const [{ data: thisWeek }, { data: prevWeek }] = await Promise.all([
    db.from("orders").select("total,status,product_name,created_at").gte("created_at", new Date(weekAgoUtc - offset).toISOString()),
    db.from("orders").select("total,status").gte("created_at", new Date(prevWeekStartUtc - offset).toISOString()).lt("created_at", new Date(weekAgoUtc - offset).toISOString()),
  ]);
