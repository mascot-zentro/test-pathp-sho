...
export function SiteNav() {
  ...
  return (
    <>
      {announcement && (
        announcement.link ? (
          <Link to={announcement.link} className="block bg-accent text-accent-foreground text-center text-sm py-2 px-4 hover:opacity-90">
            {announcement.text}
          </Link>
        ) : (
          <div className="bg-accent text-accent-foreground text-center text-sm py-2 px-4">{announcement.text}</div>
        )
      )}
